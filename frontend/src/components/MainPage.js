...
const MainPage = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState("All Products"); // Varsayılan olarak "All Products"

  const apiUrl = "https://f3837756-d355-4b7f-a67e-4ec8cdf214c7.mock.pstmn.io/getCertainProducts";
  const categoriesApiUrl = "http://localhost:5000/api/categories"; // Kategorileri almak için backend URL
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(categoriesApiUrl); // Kategorileri GET isteği ile al
        let fetchedCategories = response.data;

        // "All Products" kategorisini liste başına koymak için sıralama işlemi
        fetchedCategories.sort((a, b) => {
          if (a.name === "All Products") return -1; // "All Products" listenin başında
          if (b.name === "All Products") return 1;
          return a.name.localeCompare(b.name); // Diğer kategoriler alfabetik sıralanır
        });

        setCategories(fetchedCategories); // Kategorileri state'e ata
      } catch (error) {
